...
PASSPORT_SPECS = {
    ...
    "3.5x4.5cm": {
        "width_px": mm_to_pixels(35),  # 3.5cm = 35mm, approx 413px at 300 DPI
        "height_px": mm_to_pixels(45),  # 4.5cm = 45mm, approx 531px at 300 DPI
        "head_min_px": mm_to_pixels(29),  # approx 342 (Chin to Crown)
        "head_max_px": mm_to_pixels(34),  # approx 401
        "name": "Photo (3.5x4.5 cm)",
    },
}
...
def load_cascade():
    if not os.path.exists(CASCADE_FILE):
        ...
        print(
            f"Error: Haar Cascade file not found at {os.path.abspath(CASCADE_FILE)}"
        )  # Log to console
    ...
    try:
        ...
        if cascade.empty():
            print(
                f"Error: Failed to load Haar Cascade from {CASCADE_FILE}"
            )  # Log to console
            return None  # Indicate failure
        return cascade
...
def detect_face(image_pil, cascade):
    ...
    if cascade is None:
        st.error(
            "Face detection model (Haar Cascade) could not be loaded. Cannot process image."
        )
...
def create_passport_photo(image_pil, selected_format, cascade):
    ...
    if final_crop_w < 50 or final_crop_h < 50:
        st.warning(
            "Face detected is too close to the edge or image is too small for required crop."
        )
        return None
    ...
    try:
        ...
        final_image = cropped_image_pil.resize(
...
if face_cascade is None:
    st.error(
        f"Critical Error: Could not load face detection model (haarcascade_frontalface_default.xml). Please ensure the file exists in the same directory as the script and is not corrupted."
    )
...
selected_format_display = st.selectbox(
    "Select Passport Photo Format:", format_display_names
)
# Get the key ('US', 'UK', 'EU') from the display name
selected_format_key = format_options[
    format_display_names.index(selected_format_display)
]
...
if uploaded_file is not None:
    try:
        ...
        st.image(
            original_image, caption="Original Uploaded Photo", use_column_width=True
        )
        ...
        if st.button(f"Generate {selected_format_key} Passport Photo"):
            with st.spinner("Processing image..."):
                ...
                passport_photo = create_passport_photo(
                    original_image, selected_format_key, face_cascade
                )

            if passport_photo:
                # Store in session_state so it persists across reruns (e.g. radio button changes)
                st.session_state["passport_photo"] = passport_photo
                st.session_state["selected_format_key"] = selected_format_key
            else:
                # Clear any previous result
                st.session_state.pop("passport_photo", None)
                st.session_state.pop("selected_format_key", None)
                ...
                if (
                    detect_face(original_image, face_cascade) is None
                    and face_cascade is not None
                ):
                    st.warning(
                        "Could not detect a face in the uploaded image. Please try another photo with a clear, frontal view of the face."
                    )
                # else: a different error occurred (cascade loading, cropping issue, resize issue) - message likely shown already

        # Display generated photo and download options (persists across reruns)
        if "passport_photo" in st.session_state:
            passport_photo = st.session_state["passport_photo"]
            fmt_key = st.session_state["selected_format_key"]

            st.success("Passport photo generated successfully!")
            st.image(
                passport_photo,
                caption=f"Generated {fmt_key} Passport Photo ({PASSPORT_SPECS[fmt_key]['width_px']}x{PASSPORT_SPECS[fmt_key]['height_px']}px)",
                use_column_width=True,
            )

            # Download format selection
            download_format = st.radio(
                "Select download format:", ["PNG", "JPEG"], horizontal=True
            )

            # Prepare image for download
            buf = io.BytesIO()
            file_ext = download_format.lower()
            mime_type = f"image/{file_ext}"

            if download_format == "JPEG":
                # JPEG doesn't support transparency; convert to RGB
                download_image = passport_photo.convert("RGB")
                download_image.save(buf, format="JPEG", quality=95)
            else:
            ...
            st.download_button(
                label=f"Download Passport Photo ({download_format})",
                data=byte_im,
                file_name=f"passport_photo_{fmt_key.lower()}.{file_ext}",
                mime=mime_type,
            )
